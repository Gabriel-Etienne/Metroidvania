...
public class InteractableWater : MonoBehaviour
{
    ...
    [Header("Collision")] 
    [SerializeField, Range(1f, 20f)] private float _playerCollisionRadiusMult = 4.15f;
    
    [Header("MeshGenerator")]
    [Range(2, 500)] public int NumbOfXVertices = 70;
    ...
    private void FixedUpdate()
    {
        ...
        for (int j = 0; j < _wavePropagationIterations; j++)
        {
            for (int i = 1; i < _waterPoints.Count - 1; i++)
            {
                float leftDelta = _speedMult * (_waterPoints[i].pos - _waterPoints[i - 1].pos) * _speedMult * Time.fixedDeltaTime;
                _waterPoints[i - 1].velocity += leftDelta;
                
                float rightDelta = _speedMult * (_waterPoints[i].pos - _waterPoints[i + 1].pos) * _speedMult * Time.fixedDeltaTime;
    ...
    public void GenerateMesh()
    {
        ...
        _vertices = new Vector3[NumbOfXVertices * NUM_OF_Y_VERTICES];
        _topVerticesIndex = new int[NumbOfXVertices];

        for (int y = 0; y < NUM_OF_Y_VERTICES; y++)
        {
            for (int x = 0; x < NumbOfXVertices; x++)
            {
                float xPos = (x / (float)(NumbOfXVertices - 1)) * Width - Width / 2;
                float yPos = (y / (float)(NUM_OF_Y_VERTICES - 1)) * Height - Height / 2;
                _vertices[y * NumbOfXVertices + x] = new Vector3(xPos, yPos, 0f);

                if (y == NUM_OF_Y_VERTICES - 1)
                {
                    _topVerticesIndex[x] = y *  NumbOfXVertices + x;
                }
            }
        }
        
        int[] triangles = new int [(NumbOfXVertices - 1) * (NUM_OF_Y_VERTICES - 1) * 6];
        int index = 0;
        
        for (int y = 0; y < NUM_OF_Y_VERTICES - 1; y++)
        {
            for (int x = 0; x < NumbOfXVertices - 1; x++)
            {
                int bottomLeft  = y * NumbOfXVertices + x;
                int bottomRight = bottomLeft + 1;
                int topLeft     = bottomLeft + NumbOfXVertices;
